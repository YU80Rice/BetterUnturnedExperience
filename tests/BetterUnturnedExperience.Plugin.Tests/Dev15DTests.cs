...
namespace BetterUnturnedExperience.Plugin.Tests
{
    internal static class Dev15DTests
    {
        ...
        private static void SettingsDefaultToEnabledAndAutoRotate()
        {
            ...
            // DEV-V6-04（作废票：04）：CaptureForDrag 随策略结构迁 Bii——读取点经同一
            // 三事实 helper 还原（Enabled 恒 true=DEV-V4-04 退役语义）。
            var policy = PolicyOf(state)();
        ...
        private static void DisabledLifecycleUsesNativePassThrough()
        {
            ...
            var runtime = new BetterItemInteractionRuntime(PolicyOf(settings), lifecycle);
        ...
        {
            var settings = new BetterItemInteractionSettingsState();
            var runtime = new BetterItemInteractionRuntime(PolicyOf(settings), new BetterItemInteractionLifecycle());
            runtime.Start(true, true);
        ...
        private static void IsolationStopsCallbacksAndContinuesCleanupAfterFailure()
        {
            var lifecycle = new BetterItemInteractionLifecycle();
            lifecycle.Start(true, true);
            var runtime = new BetterItemInteractionRuntime(PolicyOf(new BetterItemInteractionSettingsState()), lifecycle);
        ...
        private static void ComponentDisabledPathReturnsNativePassThrough()
        {
            ...
            var enabledComponent = new BetterItemInteractionUiComponent(
                new InventoryPreviewPresenter(new InventoryDragPresenter(new FixedEvaluator())),
                new NativeInventoryInteractionAdapter(2, 8));
            enabledComponent.OnUiInitialized(true, false);
        ...
        /// <summary>DEV-V6-04（作废票：04）：Runtime 策略读取改注入委托——设置单事实源
        /// 留界面工程（BetterItemInteractionSettingsState），本 helper 在读取点还原
        /// 迁移前 CaptureForDrag 的三个事实（Enabled 恒 true 为 DEV-V4-04 退役语义）。</summary>
        private static Func<BetterItemInteractionDragPolicy> PolicyOf(BetterItemInteractionSettingsState state)
        {
            return () => new BetterItemInteractionDragPolicy(state.Enabled, state.AutoRotate, state.Revision);
        }
