...
namespace BetterUnturnedExperience.Plugin.Tests
{
    ...
    internal static class DevV4BiiRetireTests
    {
        ...
        private static void DisabledLifecycleNeverAutoRevivesOnDrag()
        {
            ...
            // DEV-V6-04（作废票：04）：Runtime 策略读取改注入委托（设置单事实源留界面工程）。
            var settingsState = new BetterItemInteractionSettingsState();
            var runtime = new BetterItemInteractionRuntime(
                () => new BetterItemInteractionDragPolicy(settingsState.Enabled, settingsState.AutoRotate, settingsState.Revision), lifecycle);
