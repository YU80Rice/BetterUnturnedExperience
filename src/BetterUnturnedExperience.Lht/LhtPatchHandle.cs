...
{
    /// <summary>
    /// DEV-V6-13（V6-T5 Q2 + 2026-09-18 追加裁决）：尸潮的补丁句柄——本功能的一个
    /// Harmony 身份（信标权威面 + 可选 HUD 画面面共用同一 Harmony 实例/ID）对应一枚句柄，
    /// 经启动口袋的 IFeaturePatching 进入既有资源账（同代际、同逆序释放、同每代际容量）。
    /// 句柄=BCL IDisposable（不新增契约句柄类型）；Dispose 即该补丁集的拆除动作，由平台在
    /// 停止/隔离边界按账逆序释放调用——登记成功后这就是唯一的正常拆除点，模块自身不再
    /// UnpatchSelf（禁止平台拆一次、模块再拆一次）。
    ///
    /// 与整理（LIT）版同形，另加尸潮专有的「一代内可重复全停/重臂」（设置开关 off 是全停：
    /// 追踪退订 + 频道注销 + 补丁原生回退；on = 整链重臂）：
    ...
    /// BUE-LIFE-006；释放行只在拆除动作返回后落。武装期的自拆（半装失败/缺口袋/登记被拒）
    /// 走模块自己的受守卫路径（不抛出 Start），两者分工不同（V5-04 与 05 各自纪律）。
    /// </summary>
    internal sealed class LhtPatchHandle : IDisposable
    {
        ...
        internal LhtPatchHandle(string harmonyId, Action releasePatches, Action<string> log)
        {
        ...
        /// <summary>开关热摘撤销过的次数（全停开关可重复的机器可判据；登记不因此变动）。</summary>
        ...
        public void Dispose()
        {
            ...
            if (log != null)
            {
                try { log("BUE 尸潮补丁已拆除 harmonyId=" + harmonyId + " diagnosticId=BUE-LHT-PATCH-004"); }
                catch (Exception) { }
            }
        }
    }
