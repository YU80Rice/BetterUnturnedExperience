...
namespace BetterUnturnedExperience.ClientUi.Internal
{
    ...
    internal sealed class BueNativeManagementPanel
    {
        ...
        // DEV-V6-02E: host mouths for the network takeover card and the trace
        // classifier (unbound = the card never renders, traces ride the direct
        // BepInEx source only).
        ...
        internal BueNativeManagementPanel(BueManagementPanelRuntime runtime, ManualLogSource log, IBueButtonInjectionSeam buttonInjectionSeam, System.Action refreshModel, ClientUiHostServices hostServices = null)
        {
        ...
        // DEV-V6-02E: the network card renders only when the injected host
        // identity answers (unbound = false, same as a non-network row).
        private bool IsNetworkTakeoverRow(string stableId)
        {
            var networkFeature = hostServices.GetNetworkFeature;
            if (networkFeature == null) return false;
            var feature = networkFeature();
            return feature.Value != null && string.Equals(stableId, feature.Value, StringComparison.Ordinal);
        }
        ...
        private void RenderDetails()
        {
            ...
            if (selected.Kind == ManagementEntryKind.BueFeature)
            {
                ...
                // DEV-V6-02E: the network identity crosses as an injected Func (the
                // host owns the adapter identity; unbound = the card never renders).
                if (IsNetworkTakeoverRow(selected.StableId)) RenderNetworkTakeoverDetails(ref y);
            }
            else
            {
        ...
        {
            // DEV-V6-02E: takeover state rides the injected mouths (the adapter
            // instance stays host-side; unbound = no card content).
            var takeoverStatus = hostServices.GetTakeoverStatus?.Invoke();
            var configMigration = hostServices.GetConfigMigrationStatus?.Invoke();
            if (takeoverStatus == null || configMigration == null) return;
            AddDetailLabel(ref y, "接管状态：" + takeoverStatus, ESleekFontSize.Small);
            AddDetailLabel(ref y, "配置迁移：" + configMigration, ESleekFontSize.Small);
            if (!(hostServices.IsTakeoverActive != null && hostServices.IsTakeoverActive())) return;
            ...
            revertButton.OnClicked += delegate(ISleekElement ignored)
            {
                ...
                var networkFeature = hostServices.GetNetworkFeature;
                var accepted = networkFeature != null && runtime.Model.TryToggleFeature(networkFeature(), false);
                if (accepted) hostServices.RefreshNetworkSwitches?.Invoke();
        ...
        // the heartbeat) and is OR-ed with the injected host classifier
        // (DEV-V6-02E: the verbosity policy lives host-side).
        private void LogTrace(string eventName, string details, bool isRuntime)
        {
            ...
            // DEV-V6-02E: the verbosity classifier and the Runtime mouth are
            // injected (log policy lives host-side); unbound = direct-source only.
            var isRuntimeEvent = hostServices.IsRuntimeEvent;
            if (isRuntime || (isRuntimeEvent != null && isRuntimeEvent(eventName)))
            {
                var runtimeSink = hostServices.LogRuntime;
                if (runtimeSink != null) runtimeSink(line);
                return;
            }
            if (log != null)
            {
