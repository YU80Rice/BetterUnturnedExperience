...
namespace BetterUnturnedExperience.Bii
{
    ...
    {
        private readonly Action<IInventorySurfaceContext> openDispatcher;
        ...
        internal const string LifecycleHarmonyId = "io.github.yu80rice.bue.inventory-lifecycle";
        ...
        // host log per failure stage. Tests swap in a recorder to assert the
        ...
        internal static void EmitDiagnosticOnce(string line)
        {
            ...
            // surface in the host log; the sink is a static seam kept
        ...
        internal string HarmonyId { get { return LifecycleHarmonyId; } }

        /// <summary>DEV-V6-05: true once this adapter's ORDERLY teardown skipped
        /// its own unpatch because the pocket accepted the handle — the
        /// platform's stop/isolation release owns that unpatch.</summary>
        ...
        /// set (the rig registers it before any activation; the platform owns
        /// the stop-boundary unpatch, the adapter keeps its own fail-closed
        /// unpatch on the drift paths).</summary>
        internal IDisposable PatchHandle(Action<string> log = null)
        {
            return new HarmonyPatchHandle(harmony, LifecycleHarmonyId, log);
        }
        ...
        internal InventorySurfaceLifecycleAdapter(
            Action<IInventorySurfaceContext> openDispatcher, Action closeDispatcher,
            Func<bool> isolateDispatcher = null, Action hideDispatcher = null,
            Action<byte> discardPageDispatcher = null, BiiCompositionMouths mouths = null)
        {
            ...
            harmony = new Harmony(LifecycleHarmonyId);
        ...
        internal void Activate()
        {
            ...
            try
            {
                ...
                // adapter's host log mouth so lifecycle isolations emit a
                // "reason:" line exactly once.
                DiagnosticLogSink = line => EmitErrorFriendly("[BUE-INVENTORY] event=diagnostic-failure " + line + " diagnosticId=BUE-INVENTORY-003");
                ...
                EmitRuntime("[BUE-INVENTORY] event=polling-hook-installed target=PlayerUI.Update diagnosticId=BUE-INVENTORY-001");
            }
        ...
        /// the platform's account release performs the unpatch. Every other
        /// call site (drift isolation via IsolateAndDispatch, cleanup chain)
        /// keeps the default fail-closed unpatch.</summary>
        internal bool IsolateAndDetach(bool releasePatches = true)
        {
        ...
        private void RunGuardedPoll()
        {
            ...
            try
            {
                ...
                {
                    EmitRuntime("[BUE-INVENTORY] event=surface-poll-recovered diagnosticId=BUE-INVENTORY-004");
                }
            }
            catch (Exception error)
            {
                ...
                {
                    EmitRuntime("[BUE-INVENTORY] " + failureLine);
                }
            }
        }
        ...
        // Each returns a named reason so the host log can tell which gate
        ...
        internal void Poll()
        {
            ...
            for (var pageIndex = 0; pageIndex < SupportedSurfacePages.Length; pageIndex++)
            {
                ...
                if (isolationLine != null)
                {
                    ...
                    EmitRuntime("[BUE-INVENTORY] " + isolationLine);
                }
                ...
                if (context == null)
                {
                    ...
                    {
                        if (HostIsCriticalNotReadyReason(notReadyReason))
                            EmitErrorFriendly(gateLine);
                        else
                            EmitRuntime(gateLine);
                    }
                    continue;
                }
                string readyLine;
                ...
                EmitRuntime("[BUE-INVENTORY] event=surface-context-dispatched kind=" + kind + " page=" + page + " generation=" + generation
        ...
        internal bool DiscardDispatchedSurfaceForPage(byte page, string reason)
        {
            ...
            EmitRuntime("[BUE-INVENTORY] event=surface-discarded reason=" + reason + " diagnosticId=BUE-INVENTORY-005");
        ...
        // as the 02B/C/D absent-seam contract). IsCriticalNotReadyReason is
        // the host's log-policy classifier — it decides Error vs Runtime, so
        // it crosses as a delegate instead of being duplicated here.
        ...
        private bool HostIsCriticalNotReadyReason(string reason)
        {
            var probe = hostServices != null ? hostServices.IsCriticalNotReadyReason : null;
            return probe != null && probe(reason);
        }
    }
}
