...
- 证据 A/B 所引诊断包 **20260908_210433 / 20260908_225112 / 20260908_225127**（litfb7 轮，09-08 夜）系机器侧采集、当轮消费，**未归档入库**：全库文件名搜索、git 全历史（`git log --all --name-only --diff-filter=A`）、游戏目录、TEMP 均无。
- 依据 output-review-loop「每个缺口具名」：证据 B 原始包不可复核，**其「challenge 曾送达（client 225127:1111 应用 gen=2）后链路中途劣化」的读法无法从库内原始数据重放**。
- 等效核对改用**库内归档**的 DEV-V2-24 v6/v7 P2P 主机+客机完整日志（同一 F-C 家族、同一形态、更大量化），路径见下。
