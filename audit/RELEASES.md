...
| 8 | DEV-V2-15 候选(LIT 官方纳入·单人全路径,轻量链) | `cacfa527…b040`(350208) | SHA-256 + 相邻两轮 `-t:Rebuild` 一致(`audit/2026-09-06/DEV-V2-15/identity-*.txt`;R2' 后复核不变) | 无(未提交) | 红绿链(编译红 14 错→桩运行时红→绿)+全套 7/7 PASS 0 警告+双轴 R1 双 NOT CLEAN→修复→**零上下文 R2' 双 CLEAN**(首版 R2 续用 R1 实例作废,见结单审查链) | 待用户:单人实机自验(点按钮→本地整理;关闭→原生回退;面板「背包整理」)+发布批准 | `audit/2026-09-06/DEV-V2-15/`(结单报告+红绿证据+身份) | **候选(待实机验收/批准)** |
