...
| 更好的换弹体验（LIR） | 持枪**双击换弹键**一键压弹（toast 显示压入发数）；整理后自动压弹（合并同 ID 弹匣）。弹药信息区在原版「当前/上限」旁追加**备匣 N · 备弹 M**：备匣 = 身上五页与当前枪口径匹配、有子弹的备用弹匣本数（不含枪上那本、空匣、容器、地面），备弹 = 这些备用匣余弹 + 能给当前匣供弹的弹药箱发数（与压弹匹配口径一致）。换弹技能 0～2 级：U 菜单战斗区下方追加分区（若接不上则在本功能设置页显示同一套等级），用原版经验升级——0 级基础（可双击但有额外技能冷却）、1 级快速换弹（取消额外冷却，防重复提交的技术闸仍在）、2 级自动压弹（双击成功后固定等待再自动压一轮）。**只到 2 级，没有超限弹匣** |
