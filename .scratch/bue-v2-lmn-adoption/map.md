...
- [V1 兼容路径设计](issues/V2-T4-v1-compat-path.md)：V1 兼容 = 旧插件**无改动运行**（Q1）；帧识别在 API 接收面 + 兼容策略为官方功能可关（Q2）；BUE 接管 LMN 注册入口、旧插件二进制不动（Q3）；退出度量 = 官方全 V2 + 已知插件迁移比例，阈值待 T8 生态盘点（Q4）；V1 兼容层故障只隔离 V1（Q5）；V1 帧结构代码内固化（Q6）；验收 = 旧 V1 no-op 插件不改代码能收发（Q8）；依赖 T5 先接管（Q9）。
